...
int guess() {
    char buf[16]; // 建立長度為16的字元陣列作為緩衝區
    int val, sz; // 建立整數val和sz
    printf("Show me your answer? "); // 印出提示字串
    if((sz = read(0, buf, 128)) < 0) errquit("guess/read"); // 從標準輸入讀取最多128個字元到buf中，若失敗則呼叫errquit函式
    printf("** guess: %d byte(s) read\n", sz); // 印出讀取的字元數
    if(sscanf(buf, "%d", &val) != 1) errquit("guess/sscanf"); // 從buf中讀取整數部分，若失敗則呼叫errquit函式
    return strtol(buf, NULL, 0); // 將buf轉換為長整數並回傳，若無法轉換則回傳0
}
