...
// 定义宏，如果发生错误，则输出错误信息并退出程序
#define errquit(m) { perror(m); _exit(-1); }
// 定义全局变量
static long main_min = 0, main_max = 0;//chal
static long poem_min = 0, poem_max = 0;//libpoem
// 获取进程的内存映射信息，找到程序代码段和共享库代码段的地址范围
static void get_base() {
    int fd, sz;
    char buf[16384], *s = buf, *line, *saveptr;
    
    // 如果已经获取到共享库代码段地址范围，就不再重复获取
    if(poem_max != 0) return;
    
    // 打开 /proc/self/maps 文件，获取进程的内存映射信息
    if((fd = open("/proc/self/maps", O_RDONLY)) < 0) errquit("get_base/open");

    // 读取 /proc/self/maps 文件中的内容到缓冲区中
    ...
    *s = 0;
    ...
    // 解析缓冲区中的每一行，查找程序代码段和共享库代码段的地址范围
    while((line = strtok_r(s, "\n\r", &saveptr)) != NULL) { // 用 strtok_r 函數將字串 s 分割成多行，並將每行存儲在 line 中
        s = NULL; // 重置 strtok_r 函數的第一個參數，使下次調用時可以繼續從剩下的內容中提取字串

        if(strstr(line, " r-xp ") == NULL) continue; // 如果該行字串中不包含 " r-xp "，則跳過該行

        if(strstr(line, "/libpoem.so") != NULL) { // 如果該行字串中包含 "/libpoem.so"
            if(sscanf(line, "%lx-%lx ", &poem_min, &poem_max) != 2) errquit("get_base/poem"); // 從該行字串中提取起始地址和結束地址，並存儲在 poem_min 和 poem_max 中
        } else if(strstr(line, "/chal") != NULL) { // 如果該行字串中包含 "/chal"
            if(sscanf(line, "%lx-%lx ", &main_min, &main_max) != 2) errquit("get_base/main"); // 從該行字串中提取起始地址和結束地址，並存儲在 main_min 和 main_max 中
        }

        if(main_min!=0 && main_max!=0 && poem_min!=0 && poem_max!=0) return; // 如果 poem_min、poem_max、main_min 和 main_max 都已經被設置，則退出 while 循環
    }
    _exit(-fprintf(stderr, "** get_base failed.\n")); // 如果 while 
}

// 這個函數是用來檢查目前函數的呼叫者(caller)是否為chal主函數(main function)，以確保程式的安全性。
// 這個函數會從目前的執行環境中取得呼叫者的IP位址(instruction pointer)，然後判斷這個IP位址是否在主函數的範圍內。
// 如果是，就返回0表示呼叫者為主函數；如果不是，就退出程式並輸出錯誤訊息，表示呼叫者不是主函數。
static int check() {
    int r; // declare integer variable 'r'
    unw_cursor_t cursor; // declare unw_cursor_t type variable 'cursor'
    unw_context_t uc; // declare unw_context_t type variable 'uc'
    unw_word_t ip; // declare unw_word_t type variable 'ip'
    get_base(); // call the 'get_base' function to initialize the base pointers

    // get the current execution context and initialize the cursor with it
    unw_getcontext(&uc); 
    unw_init_local(&cursor, &uc);

    // iterate through the stack frames using unw_step function
    while (unw_step(&cursor) > 0) {
        // get the instruction pointer (IP) of the current frame
        if((r = unw_get_reg(&cursor, UNW_REG_IP, &ip)) != 0) {
            // print error message if unable to get the instruction pointer
            fprintf(stderr, "u1: %s\n", unw_strerror(r));
            return -1; // return -1 to indicate error
        }
        // if the IP is within the range of the poem function, continue to next frame
        if(poem_min <= ip && ip <= poem_max) continue;
        // if the IP is within the range of the main function, return 0
        if(main_min <= ip && ip <= main_max) return 0;
        break; // break the loop if neither condition is met
    }
    // if none of the previous conditions are met, exit the program with an error message
