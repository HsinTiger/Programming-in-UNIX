...
void disassembleInstructions(void *address, size_t count, Elf64_Addr firstInsPoint, size_t code_size)
{
    ...
    // count_disassembled = cs_disasm(cshandle, (uint8_t*)address, code_size, (uint64_t)address, 0, &insn);
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        if (( long)address < ( long)Child_entryPoint || (( long)address > ( long)Child_entryPoint + text_sec_size && (insn[i].address + base_addr_offset < (unsigned long)Child_entryPoint + text_sec_size))){
                    ...
                    if (anchor_content_self.used_flag == false)
                    {
                        ...
                        for (int i = 0; i < anchor_content_self.cnt; i++) {
                            ...
                            for (int j = 0; j < len; j++) {
                                ...
                                ptrace(PTRACE_POKETEXT, child, anchor_content_self.start[i] + j*8, anchor_content_self.anchor_memory[i][j]);
                            }
                        }
